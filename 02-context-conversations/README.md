...
> @package.json が何をしているか説明して
> @README.md を要約して
> @.gitignore には何が書かれていて、なぜ必要なの？
```
...
| Pattern | できること | 使い方の例 |
|---------|--------------|-------------|
| `@file.py` | 1 つの file を参照する | `@samples/book-app-project/books.py をレビューして` |
| `@folder/` | directory 内の files 全体を参照する | `@samples/book-app-project/ をレビューして` |
| `@file1.py @file2.py` | 複数 file を同時に参照する | `@samples/book-app-project/book_app.py と @samples/book-app-project/books.py を比較して` |
...
> @samples/book-app-project/utils.py が何をするか説明して
```
...
> @samples/book-app-project/book_app.py と @samples/book-app-project/books.py を見比べて、一貫性の問題がないか確認して
```
...
> @samples/book-app-project/ 内のすべての file を見て、error handling をレビューして
```
...
>
> この app が何をするものかと、主な quality issue は何かを 1 段落でまとめて
```
...
> @samples/book-app-project/books.py を見て、潜在的な bug がないかレビューして
...
> では、@samples/book-app-project/book_app.py はどうですか？
...
> @samples/book-app-project/books.py この module は何をするの？

# Copilot CLI reads books.py and understands the BookCollection class

> @samples/book-app-project/ の code 構成を overview で教えて

# Copilot CLI scans the directory and summarizes

> この app は本をどうやって保存・読み込みしていますか？
...
> show_books() と print_books() に重複した表示処理があります。これらを統合する方法を考えてください
...
> @samples/book-app-project/ 全 module の error handling を改善したいです
...
> code quality の問題点を洗い出して、番号付きで整理して
...
> 前回の book app review で、まだ未対応の issue は何ですか？
...
> @samples/book-app-project/**/*.py の中にある TODO comment をすべて探して
```
...
> @samples/book-app-project/books.py の BookCollection class をレビューして
...
> すべての method に type hints を追加して

> 次に error handling を改善して

> この最終版に対する tests を生成して
```
...
> @samples/book-app-project/ 全体の code quality をレビューして
```
...
> /rename book-app-review
> @samples/book-app-project/books.py 空の title を防ぐ input validation を追加したいです
> その修正を実装して
> /exit

copilot --continue
> 今回の変更に対する tests を生成して
```
...
```bash
copilot
> @samples/book-app-project/books.py @samples/book-app-project/book_app.py ユーザー入力から data.json に保存されるまで、本の data がどう流れるか追ってください
> @samples/book-app-project/data.json この file が見つからない、または壊れていた場合は何が起きますか？
