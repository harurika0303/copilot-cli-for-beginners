...
```bash
> /generate-tests user authentication module の tests を作って

> /code-checklist books.py の code quality issue を確認して

> /security-audit API endpoints に vulnerability がないか確認して
```
...
```bash
> 今の返答で、どの skills を使いましたか？

> security review に使える skills は何がありますか？
```
...
```bash
> この code をレビューして。bare except 句、type hints の不足、
> mutable default arguments、file I/O の context manager 不足、
> 50 行を超える functions、production code の print statements を確認して...
```
...
> book collection の code に quality issue がないか確認して
```
...
> book app に quality issue がないか確認して
...
4. skill を試します: `@samples/book-app-project/data.json この collection の本を要約して`
5. `/skills list` や挙動で auto-trigger を確認します
6. `/book-summary この collection の本を要約して` で direct invocation も試します
...
```bash
copilot
> @samples/book-app-project/data.json この collection の本を要約して
```
...
2. 変更を stage してから「ステージした変更に対する commit message を生成して」と聞いて試す
