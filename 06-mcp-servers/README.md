...
```bash
copilot
> この repository の recent commits を表示して
```
...
**With MCP:**
```bash
> この repository の GitHub issue #42 には何が書かれていますか？
...
> Python app を Azure App Service に deploy する推奨方法を教えて。Microsoft Learn も調べて
```
...
> https://github.com/facebook/react の README を取得して要約して
```
...
> この repository の直近 5 件の commit を表示して
> この repository にはどんな branch がありますか？
> pytest を import している file をこの repository から探して
```
...
> book-app-project directory には Python file がいくつありますか？

> data.json file の合計サイズはどれくらいですか？

> book app の中で type hints が付いていない function をすべて探して
```
...
> pytest fixtures を使う best practice は何ですか？

> それを book app の test file にどう適用できますか？
```
...
> book lookup server を使って「1984」の情報を調べて。George Orwell の本も検索して
```
...
> Azure Function で managed identity を設定する方法を教えて。Microsoft Learn も調べて
```
...
> samples/book-app-project/ にある Python file をすべて列挙して、
> それぞれの役割を要約して

# Step 2: Use GitHub MCP to check recent changes
> samples/book-app-project/ の file に触れた直近 3 件の commit は何ですか？

# Step 3: Use Context7 MCP for best practices
> JSON data persistence に関する Python の best practice は何ですか？

# Step 4: Synthesize a recommendation
> book app の code とこれらの best practice を踏まえて、
> どんな改善を勧めますか？
```
...
> GitHub issue #1 の詳細を教えて

> @samples/book-app-project/books.py issue #1 に書かれている問題を修正して

> 修正が正しく動くか tests を実行して確認して

> "Add year validation to book app" という title で pull request を作成して
```
...
> book app project の health report を作ってください:
> 1. samples/book-app-project/ の Python file にある function をすべて列挙する
> 2. type hints がある function とない function を確認する
> 3. samples/book-app-project/tests/ にどんな tests があるか示す
> 4. この directory の recent commit history を確認する
```
...
> samples/book-app-project/ には Python file がいくつありますか？
> 各 file にはどんな function が定義されていますか？
```
...
> samples/book-app-project/data.json を読んで、collection にどんな本があるか教えて。
> そのあと recent commits を確認して、この file が最後に更新された時期も教えて
```
...
| Mistake | 起きること | Fix |
|---------|--------------|-----|
| GitHub MCP が built-in だと知らない | 手動インストールしようとしてしまう | まずは「この repo の recent commits を表示して」を試す |
