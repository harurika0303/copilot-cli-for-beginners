...
```
> Python の dataclass とは何かをやさしく説明して

> 特定の key で辞書の list を並べ替える function を書いて

> Python の list と tuple の違いは？

> 読みやすい Python code の best practice を 5 つ教えて
```
...
**ここでのポイント**: GitHub Copilot CLI は conversational です。特別な記法を覚えなくても、まずは自然な日本語で質問するだけで始められます。
...
```
> @samples/book-app-project/book_app.py をレビューして、code quality の問題点と改善案を教えて
```
...
```
> @samples/book-app-project/books.py が何をしているかをやさしく説明して
```
...
**何が起きるか**: （実際の出力は異なりますが）Copilot CLI が file を読み取り、コードを理解し、やさしい言葉で説明してくれます。
...
```
> 本の list を受け取って統計を返す Python function を書いて:
  総数、既読数、未読数、最も古い本、最も新しい本
```
...
> @samples/book-app-project/utils.py をレビューして改善案を出して

> すべての function に type hints を追加して

> error handling をより堅牢にして
...
> /plan book app に "mark as read" command を追加したい
```
...
```bash
# Generate code
copilot -p "数値が偶数か奇数かを判定する function を書いて"

# Get quick help
copilot -p "Python で JSON file を読む方法は？"
```
...
COMMIT_MSG=$(copilot -p "次の差分に対する commit message を生成して: $(git diff --staged)")
git commit -m "$COMMIT_MSG"

# Review a file
copilot --allow-all -p "@myfile.py に issue がないかレビューして"
```
...
> 💡 **`/ask` と通常の chat の違い**: 普通は送ったメッセージが会話の一部として残り、後続の返答にも影響します。`/ask` は「記録に残さず聞く」ための shortcut です。たとえば `/ask YAML とは何ですか？` のように、セッションの context を汚さずに軽く確認できます。
...
> @samples/book-app-project/book_app.py を見て、改善できる点を教えて

> if/elif の連鎖を、より保守しやすい構造に refactor して

> すべての handler function に type hints を追加して
...
> /plan book app に、title または author で本を検索できる search feature を追加したい
...
for file in samples/book-app-project/*.py; do
  echo "Reviewing $file..."
  copilot --allow-all -p "@$file を簡潔にコードレビューして。critical issues のみ教えて"
done
```
...
Get-ChildItem samples/book-app-project/*.py | ForEach-Object {
  ...
  copilot --allow-all -p "@$relativePath を簡潔にコードレビューして。critical issues のみ教えて" 
}
```
...
2. **Plan Mode Challenge**: `/plan book app に rating と review 機能を追加したい` を実行します。plan をよく読んで、筋が通っているか確かめてください。

3. **Programmatic Challenge**: `copilot --allow-all -p "@samples/book-app-project/book_app.py にあるすべての function を列挙して、それぞれの役割を説明して"` を実行します。1 回でうまくいきましたか？
...
2. Copilot CLI に file の要約を頼みます: `@samples/book-app-project/utils.py この file の各 function が何をするか説明して`
3. input validation を追加してもらいます: "`get_user_choice()` が空入力や数字以外の入力も扱えるように validation を追加して"
4. error handling を改善してもらいます: "`get_book_details()` が title に空文字を受け取ったらどうなりますか？ そのための guard を追加して"
5. docstring を追加してもらいます: "`get_book_details()` に parameter と return value を含む詳しい docstring を追加して"
...
**試せる sample prompt:**
```bash
> @samples/book-app-project/utils.py この file の各 function が何をするか説明して
> get_user_choice() が空入力や数字以外の入力も扱えるように validation を追加して
> get_book_details() が title に空文字を受け取ったらどうなりますか？ そのための guard を追加して
> get_book_details() に parameter と return value を含む詳しい docstring を追加して
```
...
2. **Plan**: `/plan publication year の範囲で本を絞り込む list_by_year(start, end) method を BookCollection に追加したい`
3. **Programmatic**: `copilot --allow-all -p "@samples/book-app-project/books.py に、start 年から end 年までに出版された本を返す list_by_year(start, end) method を追加して"`
...
| `$` や `!` を含む prompt を quote しない | Copilot CLI に渡る前に shell が特殊文字を解釈してしまう | `copilot -p "$HOME とは何ですか？"` のように quote で囲む |
